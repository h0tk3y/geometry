...
object BentleyOttmanIntersection : IntersectionProvider {
    ...
    override fun intersection(segments: List<Segment>): List<Point> {
        ...
        while (events.isNotEmpty()) {
            ...
            sweepY = prevY + (p.y - prevY) / 2

            fun checkIntersection(s1: Segment?, s2: Segment?) {
                if (s1 != null && s2 != null) {
                    val i = intersectionPoint(s1, s2)
            ...
            sweepY = p.y + ((nextEvent?.y ?: 1.0) - p.y) / 2
            sweepX = p.x + ((nextEvent?.x ?: 0.0) - p.x) / 2
            ...
            for (t in toAdd) {
                ...
                checkIntersection(t, left)
                checkIntersection(t, right)
            }
            status removeAll toAdd
            for (t in toAdd) {
                status add t
                val left = status.headSet(t).lastOrNull { it !in toAdd }
                val right = status.tailSet(t).firstOrNull { it !in toAdd }
